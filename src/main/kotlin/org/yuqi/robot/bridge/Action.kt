...
enum class Action(val component: ActionComponent?, val value: Double) : IIndexable {
    ...
    /*TurretLeftHalf(ActionComponent.Turret, Rules.GUN_TURN_RATE_RADIANS / 2),
    TurretRightHalf(ActionComponent.Turret, -Rules.GUN_TURN_RATE_RADIANS / 2),
    TurretLeftFull(ActionComponent.Turret, Rules.GUN_TURN_RATE_RADIANS),
    TurretRightFull(ActionComponent.Turret, -Rules.GUN_TURN_RATE_RADIANS),*/
    ...
    fun applyTo(robot: AdvancedRobot) = with(robot) {
        if (component == null)
            return
        when (component) {
            ActionComponent.Engine       -> setAhead(if (value == 0.0) 0.0 else value.sign * Double.MAX_VALUE)
